...
| Event Program Breakdown | Compare outcomes across programs | Per program: Present, Late, Absent counts and rate | Event, Program | Admin, Campus Admin, SSG/SG/ORG (with `manage_attendance`, scoped) | `get_event_attendance_report` (`GET /api/attendance/events/{event_id}/report`) | `Backend/app/routers/attendance/reports.py` | School IT |
...
| Event No Sign-Out Absence Report | Detect sign-out process gaps | No-sign-out rows count as Absent because Present/Late require sign-out | Event | Admin, Campus Admin, SSG/SG/ORG (with `manage_attendance`, scoped) | Derived from `get_event_attendance_report` (`GET /api/attendance/events/{event_id}/report`) | `Backend/app/routers/attendance/reports.py` | School IT / SSG |
...
| Student Attendance Performance Card | Full summary for one student | Attended, Late, Absent, Excused, Rate | Student, Date Range, Event Type | Admin, Campus Admin, SSG/SG/ORG (with `manage_attendance`, scoped), Student (own report only) | `get_student_attendance_report` (`GET /api/attendance/students/{student_id}/report`) | `Backend/app/routers/attendance/reports.py` | School IT / Advisers |
...
| Event Execution Quality Report | Detect attendance flow problems | No-sign-out absence count and late sign-out pattern | Event, Date Range | Admin, Campus Admin, SSG/SG/ORG (with `manage_attendance`, scoped) | School IT |
...
| School KPI Dashboard Report | Executive overview | Attendance Rate, Late Rate, Absent Rate, Participation Reach | Date Range, Department, Program | Admin, Campus Admin, SSG (school-wide visibility), SG/ORG (scoped view) | Campus Admin |
