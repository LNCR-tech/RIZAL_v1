...
watch(lastItPasswordReset, (value) => {
  if (value?.temporary_password) {
    pushFeedback('success', `Temp password for ${value.email}: ${value.temporary_password}`)
  }
})
...
async function handleResetPassword(account) {
  if (!confirm(`Reset password for ${formatPersonName(account.first_name, account.last_name)}?`)) return
  try {
...
function formatPersonName(firstName, lastName) { return [firstName, lastName].filter(Boolean).join(' ').trim() || 'Admin' }
