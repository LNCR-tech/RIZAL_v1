...
## 2026-04-17 - Fix: Recover broken db.json and standardize mock API route prefixes

### Purpose

Fixed a critical "Expected array" JSON parsing error in `db.json` and resolved inconsistent route matching in `mock-api.js`. Standardized the student import preview response to match the frontend expected `rows` key.
...
- **Data Recovery**: Restored `db.json` structure by re-adding missing commas and removing the invalid `$schema` key.
- **Route Standardization**: Removed `/api` prefix from route registrations in `mock-api.js` to align with the global middleware that strips the prefix before matching.
- **Import Preview Fix**: Changed `sample_rows` to `rows` in the student import preview response to match the frontend consumer's logic.

### How to test

1. Restart the mock server: `node mock-api.js`.
2. Confirm the server starts without "Expected array" or "SyntaxError" errors.
3. Perform a student import preview and confirm the rows are displayed correctly in the UI.
